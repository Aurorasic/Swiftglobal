...
public class ServerInboundHandler extends BaseInboundHandler {
    ...
    public ServerInboundHandler(ApplicationContext context, Channel channel) {
        super(context, channel);
    }

    @Override
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
        //check the number of p2p channels
        if (channelMgr.countAllChannels() >= appConfig.getMaxInboundConnections()) {
            LOGGER.warn("Over the max inbound connections");
            ctx.close();
            return;
        }
    }
    ...
    protected void channelRead0(ChannelHandlerContext ctx, BaseMessage msg) throws Exception {
        ...
        LOGGER.warn("Message: [{}], channel id: {}", MessageType.of(msg.getCmd()), channel.getId());
